...
export default function LeftBarNav() {
    ...
    function navLeftBarDesktop() { 
        return (         
        <aside className={`left-bar flex flex-col transition-all duration-500 ${animationClass} left-[1%] top-[6%] xs:top-[5%] md:top-[4%] lg:top-[40%] text-xs md:text-base`}>      
            <nav>
        ...
        return (
            ...
            <ListCollapse className="accordion-icon relative" onClick={() => setCurrentState(!currentState)}/>             
                <aside className={`left-bar flex flex-col transition-all duration-500 left-[1%] top-[6%] xs:top-[5%] md:top-[4%] lg:top-[40%] ${backgroundMobileTablet} text-xs md:text-base ${currentState ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2 pointer-events-none"}`}>                               
                    <nav>
                        <ul className='left-bar-nav flex flex-col items-left gap-2'> 
                            <li className={activeSection === "inicio" ? "active-left-nav" : "inactive-left-nav"} onClick={() => setCurrentState(!currentState)}>
                                <a href=""><HouseHeart size={icon_size}/></a>INICIO  
                            </li>
                            <li className={activeSection === "sobreMi" ? "active-left-nav" : "inactive-left-nav"} onClick={() => setCurrentState(!currentState)}>
                                <a href=""><UserStar size={icon_size}/></a>SOBRE MÍ
                            </li>
                            <li className={activeSection === "experiencia" ? "active-left-nav" : "inactive-left-nav"} onClick={() => setCurrentState(!currentState)}>
                                <a href=""><BriefcaseBusiness size={icon_size}/></a>EXPERIENCIA
                            </li>                    
                            <li className={activeSection === "tonextaxis" ? "active-left-nav" : "inactive-left-nav"} onClick={() => setCurrentState(!currentState)}> 
                                <a href=""><CirclePlay size={icon_size}/></a>TONEXTAXIS 
                            </li>
                            <li className={activeSection === "blog" ? "active-left-nav" : "inactive-left-nav"} onClick={() => setCurrentState(!currentState)}>
                                <a href=""><NotebookPen size={icon_size}/></a>BLOG
                            </li>                    
                        </ul>                                       
                     </nav>       
